...
This script will create all teachers, subjects, rooms, and schedule entries
"""
import asyncio
import sys
import os
...
async def create_real_timetable():
    ...
    try:
        ...
        # 1. Create Department and Group
        logger.info("📚 Creating Department and Group...")
        
        # Create or get existing group (LI 02, LI 04, etc.)
        groups_to_create = [
            {"name": "LI 02", "level": "L2"},
            {"name": "LI 04", "level": "L2"}, 
            {"name": "LI 05", "level": "L2"},
        ...
        for room_data in rooms_to_create:
            try:
                ...
                logger.warning(f"⚠️ Room {room_data['name']} might exist: {e}")
                existing_room = db.query(Room).filter(Room.name == room_data["name"]).first()
                if existing_room:
                    created_rooms[room_data["name"]] = existing_room
        ...
        for teacher_data in teachers_to_create:
            ...
            if not existing_user:
                ...
                existing_teacher = db.query(Teacher).filter(Teacher.user_id == existing_user.id).first()
                if existing_teacher:
        ...
        existing_student_user = db.query(User).filter(User.email == "student.li04@univ.tn").first()
        ...
        logger.info("   Teachers: [name]@univ.tn / teacher123")
        logger.info("=" * 60)
