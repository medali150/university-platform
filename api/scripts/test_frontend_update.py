#!/usr/bin/env python3
"""
Test the schedule update with the exact same payload as frontend
"""
import requests
import json

def test_frontend_like_update():
    """Test updating a schedule with the same payload structure as frontend"""
        ...
        if login_response.status_code == 200:
            ...
            if schedules_resp.status_code == 200:
                ...
                if schedules:
                    ...
                    print(f"   Current date: {schedule.get('date')}")
                    print(f"   Current start time: {schedule.get('heure_debut')}")
                    print(f"   Current end time: {schedule.get('heure_fin')}")
                    
                    # Simulate frontend payload - same date, different times
                    update_data = {
                        "date": schedule.get('date'),  # Same date
                        "start_time": "16:00",
                        "end_time": "17:30",
                    ...
                    print(f"\n📝 Updating schedule with frontend-like payload:")
